...
| **OpenRouter API (`openai/gpt-4o-mini`)** | AI-движок | Экономичная модель с высоким качеством для задач генерации и оценки |
...
```
server.js
├── POST /api/journey/generate    # Генерация Journey (SSE-стриминг)
├── POST /api/evaluate            # AI-оценка ответов (SSE-стриминг)
...
Пользователь вводит вопрос
  ...
  → OpenRouter: RAG_SEARCH_PROMPT + контекст + вопрос
  → JSON { answer, sources[] }
...
JourneyNewPage (или ?topic= от рекомендаций)
  → POST /api/journey/generate  (SSE → OpenRouter)
...
# 3. Добавьте API-ключ (OpenRouter)
...
- **API-ключ**: хранится только на сервере (`process.env.OPENAI_API_KEY`)
